...
Shader "Shader Forge/FogShader" {
    Properties {
        ...
        _radialfog ("radial fog", Range(0.1, 5)) = 1.64357
        _lens ("lens", Range(-1, 0)) = 0
    }
    SubShader {
        Tags {
            "Queue"="Geometry+1"
...
////// Emissive:
                ...
                float node_1589 = (1.0 - pow(length(node_1363),_lens));
                float2 node_9185 = (i.uv0+(node_1363*clamp(node_1589,(-0.5),0.0)));
                float2 node_8397 = node_9185; // Refract here
                float4 node_1672 = tex2D(_MainTex,TRANSFORM_TEX(node_8397, _MainTex));
                float node_4104 = saturate(((1.0+pow((length((i.uv0*1.0+-0.5))*2.0+0.0),_radialfog))*saturate((sceneZ-partZ)/_depth)));
